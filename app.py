...
# ============================================================
# 🧠 AyurVoice AI — Ayurvedic Medicine Voice Recognition (Fast Dropbox Edition)
# ============================================================
...
def ensure_folder_structure():
    """Create /AyurVoice folders if missing"""
    dbx = connect_dropbox()
    if not dbx: return
    folders = ["recordings", "new_samples", "models", "feedback", "mfccs"]
    for f in folders:
        path = f"/AyurVoice/{f}"
        try:
            dbx.files_get_metadata(path)
...
# ============================================================
# 🎧 AUDIO PROCESSING + MFCC CACHING
# ============================================================

def extract_mfcc_from_bytes(audio_bytes, sr=16000, n_mfcc=20):
    """Compute MFCC mean vector from WAV bytes"""
    y, _ = librosa.load(io.BytesIO(audio_bytes), sr=sr)
    y, _ = librosa.effects.trim(y)
    mfcc = librosa.feature.mfcc(y=y, sr=sr, n_mfcc=n_mfcc)
    return np.mean(mfcc, axis=1)

def get_or_create_mfcc(file_name, folder):
    """Use cached MFCC or create new one"""
    mfcc_name = file_name.replace(".wav", ".npy")
    mfcc_bytes = download_dropbox_file("mfccs", mfcc_name)
    if mfcc_bytes:
        return np.load(io.BytesIO(mfcc_bytes.read()))
    wav_bytes = download_dropbox_file(folder, file_name)
    if not wav_bytes: return None
    mfcc = extract_mfcc_from_bytes(wav_bytes.read())
    mfcc_buf = io.BytesIO()
    np.save(mfcc_buf, mfcc)
    upload_bytes_to_dropbox(mfcc_buf.getvalue(), mfcc_name, "mfccs")
    return mfcc
...
def train_svm_from_dropbox():
    """Retrain SVM using both reference & new_samples"""
    dbx = connect_dropbox()
    if not dbx: return None, 0, 0
    ...
    X, y = [], []
    for f in all_files:
        folder = "new_samples" if f in list_dropbox_files("new_samples") else "recordings"
        mfcc = get_or_create_mfcc(f, folder)
        if mfcc is not None:
            label = "_".join(f.replace(".wav", "").split("_")[:-1]) or f.replace(".wav", "")
            if f"{folder}/{f}" in feedback_map:
                label = feedback_map[f"{folder}/{f}"]
            X.append(mfcc)
            y.append(label.lower())
    ...
    model_buf.seek(0)
    upload_bytes_to_dropbox(model_buf.read(), "svm_model.joblib", "models")
...
# ============================================================
# 🔍 RECOGNITION (FAST COSINE SIMILARITY)
# ============================================================
...
def recognize_from_dropbox(audio_data):
    ...
    input_vec = extract_mfcc_from_bytes(audio_data.getvalue())
    similarities = []

    for ref in refs:
        ref_vec = get_or_create_mfcc(ref, "recordings")
        if ref_vec is None:
            continue
        score = cosine_similarity([input_vec], [ref_vec])[0][0]
        base = "_".join(ref.replace(".wav", "").split("_")[:-1]) or ref.replace(".wav", "")
        similarities.append((base.replace("_", " "), score))

    similarities.sort(key=lambda x: x[1], reverse=True)
    top3 = similarities[:3]
    pred = top3[0][0]

    # Save test sample for learning
    new_name = f"{pred}_{len(list_dropbox_files('new_samples'))+1}.wav"
    upload_bytes_to_dropbox(audio_data.getvalue(), new_name, "new_samples")
    mfcc_buf = io.BytesIO()
    np.save(mfcc_buf, input_vec)
    upload_bytes_to_dropbox(mfcc_buf.getvalue(), new_name.replace(".wav", ".npy"), "mfccs")

    result = "🎯 **Top 3 Matches (Cosine Similarity):**\n"
    for n, s in top3:
        result += f"- {n}: {s*100:.2f}%\n"
    result += f"\n✅ **Predicted Medicine:** {pred}"
    return result, pred
...
st.set_page_config(page_title="AyurVoice AI — Fast Dropbox", layout="wide")
st.title("🧠 AyurVoice AI — Ayurvedic Medicine Voice Recognition (Fast Dropbox Edition)")
st.caption("Optimized with MFCC caching, cosine similarity, and cloud retraining")

ensure_folder_structure()
tab1, tab2 = st.tabs(["🎙️ Record Reference", "🔍 Recognition & Feedback"])

# TAB 1 — Reference Recording
with tab1:
    st.subheader("Add New Medicine Reference Samples")
    name = st.text_input("Enter Medicine Name:")
    audio_data = st.audio_input("🎧 Record or Upload Reference Audio", sample_rate=16000)
    if st.button("Save Reference"):
        if name and audio_data:
            file_name = f"{name.strip().replace(' ', '_').lower()}_{len(list_dropbox_files('recordings'))+1}.wav"
            upload_bytes_to_dropbox(audio_data.getvalue(), file_name, "recordings")
            mfcc = extract_mfcc_from_bytes(audio_data.getvalue())
            mfcc_buf = io.BytesIO()
            np.save(mfcc_buf, mfcc)
            upload_bytes_to_dropbox(mfcc_buf.getvalue(), file_name.replace(".wav", ".npy"), "mfccs")
            st.success(f"✅ Saved `{name}` to Dropbox/recordings/")
        else:
            st.warning("⚠️ Enter a name and record/upload audio.")
...
st.markdown("---")
st.caption("© 2025 AyurVoice Project | Cloud-only • Fast MFCC Matching • Feedback-driven Learning")
